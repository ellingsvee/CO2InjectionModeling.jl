using CO2BatchFill: SimulatorState, SIMULATOR

export load_sleipner_topography, create_domain_from_topography
export generate_reservoir_properties_for_sleipner_layers
export generate_fitted_reservoir_properties_for_sleipner_layers
export generate_sleipner_injection_events
export parse_zmap_polygon, polygon_centroid, utm_to_grid_index, load_feeder_location
export setup_sleipner_simulator, setup_sleipner_reservoir

"""
    load_sleipner_topography(path::String="sleipner/depth_surfaces/")

Load Sleipner topography from NPZ depth surface files.

# Arguments
- `path`: Path to directory containing depth surface .npy files
...
"""
    create_domain_from_topography(topography::SleipnerTopography, dz::Float64)

Create a 3D domain from Sleipner topography. This is a Sleipner-specific wrapper
that calls the generic `create_domain` function.
...
# Returns
- `Domain3D`: 3D domain for simulation
"""
function create_domain_from_topography(topography::SleipnerTopography, dz::Float64)::Domain3D
    # Delegate to the generic create_domain function
    return CO2BatchFill.create_domain(topography, dz)
end
...
# =============================================================================
# R interface convenience wrappers
# =============================================================================

"""
    setup_sleipner_simulator(; data_path="sleipner/depth_surfaces/", boundary_condition="open")

Load Sleipner topography and set up the simulator.

Convenience wrapper that loads Sleipner data and calls the generic
`CO2BatchFill.setup_simulator()`, then sets Sleipner default reservoir properties.
"""
function setup_sleipner_simulator(;
    data_path::String="sleipner/depth_surfaces/",
    boundary_condition::String="open"
)
    try
        topo = load_sleipner_topography(data_path)
        result = CO2BatchFill.setup_simulator(topo; boundary_condition=boundary_condition)
...
Must call `setup_sleipner_simulator()` first.
"""
function setup_sleipner_reservoir()
    try
        if isnothing(SIMULATOR.layers)
            return Dict("status" => "error", "message" => "Must call setup_sleipner_simulator first")
        end
        SIMULATOR.reservoir_properties = generate_reservoir_properties_for_sleipner_layers()
        return Dict("status" => "success", "n_layers" => length(SIMULATOR.layers))
    catch e
        return Dict("status" => "error", "message" => string(e))
    end
end

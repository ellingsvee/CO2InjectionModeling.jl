...
function test_full_simulation_mass_conservation()
    @testset "Mass conservation for a full simulation" begin
        ...
        injection_loc_1 = (NX - div(NX, 4), NY - div(NY, 4))
        injection_loc_2 = (div(NX, 4), div(NY, 4))
        rate_L1_1 = create_injection_rate(layers, injection_loc_1, TOTAL_RATE; radius=25)
        rate_L1_2 = create_injection_rate(layers, injection_loc_2, TOTAL_RATE; radius=25)
        rate_L1 = rate_L1_1 .+ rate_L1_2

        # rate_L1 = create_injection_rate(layers, injection_loc_1, TOTAL_RATE; radius=15)
        ...
        seqs, leakage_states, weather_events_per_layer = fill_layers(
            layers, domain, all_layers_rp, injection_events; verbose=false, leakage_radius=25)

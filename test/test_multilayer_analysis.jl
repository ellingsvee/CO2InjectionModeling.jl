function run_multilayer_analysis_tests(scenario::TestScenario)
    @testset "MultiLayerSnapshot: mass conservation and consistency" begin
        ...
        # ── System-level mass conservation ───────────────────────────────────
        @testset "System conservation" begin
            for msnap in multi_snaps
                @test msnap.total_stored + msnap.total_surface_leakage ≈ msnap.total_injected atol = MASS_ATOL
            end
        end

    ...
    try
        ...
        SIMULATOR.reservoir_properties = [
            ReservoirProperties(
                ...
                shale_pressure_threshold[i], leakage_height[i], residual_leakage_time[i]
            )
            for i in 1:n_layers
        ]
...
function run_simulation(;
    ...
    try
        ...
        snapshots = generate_reservoir_snapshots(
            SIMULATOR.layers, seqs, leakage_states, SIMULATOR.domain,
            SIMULATOR.reservoir_properties, injection_events;
            num_snapshots=num_snapshots, start_time=start_time, end_time=end_time, verbose=verbose
        )
        ...
        return Dict(
            ...
            "layer_co2_volumes" => hcat([s.stored_by_layer_m3 for s in snapshots]...)',
            "num_layers" => n_layers,
